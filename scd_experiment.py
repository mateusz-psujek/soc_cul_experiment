...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\sync\\experiment_social_cultural_exam\\scd_experiment\\scd_experiment.py',
...
# --- Initialize components for Routine "remove_me" ---
...
# --- Prepare to start Routine "remove_me" ---
continueRoutine = True
# update component parameters for each repeat
# keep track of which components have finished
remove_meComponents = [circle]
for thisComponent in remove_meComponents:
...
# --- Ending Routine "remove_me" ---
for thisComponent in remove_meComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# the Routine "remove_me" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()
...
for thisLoop_condition_spec in loop_condition_spec:
    ...
    loop_condition_spec.addData('key_resp_ratio_bias_task.keys',key_resp_ratio_bias_task.keys)
    if key_resp_ratio_bias_task.keys != None:  # we had a response
        loop_condition_spec.addData('key_resp_ratio_bias_task.rt', key_resp_ratio_bias_task.rt)
    # check responses
    if key_confirm_ratio_bias_task.keys in ['', [], None]:  # No response was made
        key_confirm_ratio_bias_task.keys = None
    loop_condition_spec.addData('key_confirm_ratio_bias_task.keys',key_confirm_ratio_bias_task.keys)
    if key_confirm_ratio_bias_task.keys != None:  # we had a response
        loop_condition_spec.addData('key_confirm_ratio_bias_task.rt', key_confirm_ratio_bias_task.rt)
